...
void main(){
	...
	while (true){	
		temp_float = ds18b20_read(); //Doc nhiet do tu cam bien DS18B20
		...
		if (E0 == 0){
			...
			i = 0;
			position = 64;			
			B_EXIT = input(SETUP_EXIT);	//Khi chua nhan nut EXIT hay ENTER 
			B_ENTER = input(ENTER);
			while ((B_ENTER==1) | (B_EXIT == 1)){
				...
				if ((value >= 0) && (value < 10) && (i<=2)){
					...
					if ((value == 100) && (i>=3)){
						...
						if (value == 50){ 	//Khi da nhan EXIT thoat khoi vong lap
							LCD_PutCmd(0x01);							
							LCD_SetPosition(0x00);	//Chuyen vi tri con tro sang dong 1
